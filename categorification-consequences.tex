...
\section{Quantum Cluster Algebras and Quantum Cluster Characters}
  ...
  Define a quiver $Q$ with vertices $\{1,\ldots,n\}$ and $\gcd(b_{ij},b_{ji})$ arrows $i\longrightarrow j$ whenever $b_{ij}<0$, we assume this quiver to be acyclic.
  Write $\tilde Q$ for the quiver obtained from $Q$ by attaching to each vertex $i$ an additional vertex $n+i$ via a single arrow $i\to n+i$.  
