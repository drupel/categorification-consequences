...
\label{sec:qca}
  ...
  \sayS{I was really tempted to use $[\cdot]_+$ here but I fear it is going to generate confusion with classes.}
...
\section{Deducing the Conjectures}
  ...
  \sayD{Fill this out a little.}
   ...
   \begin{proposition}
    ...
    \sayS{This proposition has not been cleaned yet.}
  \end{proposition}
  \begin{proof}
  ...
  \begin{corollary}[{\cite[Conjecture 7.17]{fomin-zelevinsky4} for acyclic initial exchange matrices}]
    The denominator vector of $x_\bfv$ is the exponent vector of the monomial obtained by tropically evaluating the corresponding $F$-polynomial at $y_1^{-1},\dots,y_n^{-1}$.
  \end{corollary}
  \begin{proof}
    ...
    Indeed, this is the monomial corresponding to the full subrepresentation and hence it has exponent vector $\bfv$.
  \end{proof}
  ...
  \begin{corollary}
    ...
    \begin{enumerate}
      ...
      \item
        $v_i$ depends only on $x_\bfv$ and $x_i$ but not the seed containing $x_\bfv$.
        \sayS{This might be independent also of the acyclic initial seed containing $x_i$}
    \end{enumerate}
  \end{corollary}
  \begin{proof}
  ...
  \begin{corollary}[{\cite[Conjecture 7.6]{fomin-zelevinsky4}} for acyclic initial exchange matrices]
    ...
    In $\cA(\bfx,\widetilde{B})$ different cluster monomials have different $\bfd$-vectors.
    Moreover, the $\bfd$-vectors of the cluster variables of any given cluster form a $\ZZ$-basis of $\cQ$.
  \end{corollary}
  \begin{proof}
    By Corollary \ref{cor:sign_coherence}, if a cluster monomial contains initial cluster variables this can be detected in the $\bfd$-vector.
  ...
  \end{proposition}
  \begin{proof}
    Since the $\bfg$-vector of the cluster variable $x_\bfv$ is given by $-{}^*\bfv$, the result follows immediately from the definitions of $E$ and the operator ${}^*(-)$.
  \end{proof}
  
  \begin{corollary}[{\cite[Conjectures 7.2 and 7.10]{fomin-zelevinsky4}} for acyclic initial exchange matrices]
    In $\cA(\bfx,\widetilde{B})$ different cluster monomials have different $\bfg$-vectors.
    Moreover, the $\bfg$-vectors of the cluster variables of any given cluster form a $\ZZ$-basis of $\ZZ^n$.
    Finally cluster monomials are linearly independent.
  \end{corollary} 
  \begin{proof}
    Proposition \ref{prop:d to g} can easily be extended to cover initial cluster variables by replacing the matrix~$E$ with the piecewise linear map $\nu_c$ given in \cite{reading-stella-2}.
    The first two claims follow immediately from Corollary \ref{cor:d_vect basis}.
    The last follows as explained in \cite[Remark 7.11]{fomin-zelevinsky4}.
  \end{proof} 
  ...
  \end{corollary}
  \begin{proof}
    By Theorem~\ref{th:reflection functor}, the initial cluster mutation at a sink or a source vertex transforms quantum cluster characters according to the associated reflection functor on $\rep_{\FF}(Q,D)$.
    The result then follows from \cite[Prop. 2.1]{dlab-ringel} and Corollary \ref{cor:classical cluster characters}.
  \end{proof}
  ...
  \begin{proof}
    ...
    Since vertex $k$ is a source in $B$ we have $b_{kj}\leq 0$ for $j\in[1,n]$ and therefore only the exponent of $y_k$ could end up being negative in the evaluation.
  ...
  \begin{proposition}[{\cite[Conjecture 3.10]{reading-speyer}} for acyclic initial exchange matrices]
    \sayS{We could probably formulate this for any initial exchange matrix mutationally equivalent to an acyclic one but notation would be bad, and the results can be deduced in half a line if needed}
    The induced subgraph of the exchange graph of $\cA(\bfx,\widetilde{B})$ consisting of all the seeds containing any fixed collection of cluster variables is connected.
  \end{proposition}
  \begin{proof}
  ...
  \end{proposition}
  \begin{proof}
    The proof of the analogous statement for acyclic skew-symmetric cluster algebras given in \cite[Corollary 4]{caldero-keller2} is also written in terms of arbitrary hereditary abelian categories and thus it applies in our situation as well.
  \end{proof}
  
  \sayS{Try to prove that $\bfc$-vectors are roots following the same procedure of Najera-Chavez}
