...
\section{Quantum Cluster Algebras and Quantum Cluster Characters}
  ...
  \begin{theorem}\cite{rupel1,rupel2}
    ...
    \begin{enumeratea}
      ...
      \item 
        For each rigid representation $V\in\rep_\FF(Q,D)$ and each $\bfe\in\cQ$, there exists a polynomial $P_{\bfv,\bfe}(q)$, depending only on the dimension vector $\bfv$ of $V$, so that $\big|Gr_\bfe(V)\big|=P_{\bfv,\bfe}\big(|\FF|\big)$.
