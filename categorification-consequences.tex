...
\section{Quantum Cluster Algebras and Quantum Cluster Characters}
  ...
  We write $D$ for the diagonal matrix whose~$ii$ entry is $d_i$ and fix such a matrix once and for all.
  ...
  Then the \newword{quantum cluster character} of a representation $V\in\rep_\FF(Q,D)$ with $[V]=:\bfv$ is given by 
  \[
    X_V
    :=
    \sum\limits_{\bfe\in\cQ} |\FF|^{-\half\langle\bfe,\bfv-\bfe\rangle}\big|\!\Gr_\bfe(V)\big|X^{\widetilde{B}\bfe-{}^*\bfv}\in\cT_\Lambda,
  \] 
  where $\Gr_\bfe(V)$ denotes the set of all subrepresentations $E\subset V$ with isomorphism class $[E]=\bfe$.  
  Note that $\widetilde{B}\bfe={}^*\bfe-\bfe^*$ and, for $j\in[1,n]$, we have ${}^*\alpha_j=\alpha_j+\sum\limits_{i:b_{ij}<0}b_{ij}\alpha_i$.
  ...
  \begin{theorem}\cite{rupel1,rupel2}
    ...
    \begin{enumeratea}
      \item For each rigid representation $V\in\rep_\FF(Q,D)$, the quantum cluster character $X_V$ is a quantum cluster monomial of $\cA_{|\FF|}(\bfX,\widetilde{B})$.  
      Moreover, every non-initial cluster variable of $\cA_{|\FF|}(\bfX,\widetilde{B})$ arises in this way from a rigid indecomposable representation.
      ...
      which computes the cluster variables of $\cA_q(\bfX,\widetilde{B})$ with arbitrary parameter $q$.
    \end{enumeratea}
  \end{theorem}
  ...
  \begin{corollary}
    ...
    All non-initial cluster variables of the cluster algebra $\cA(\widetilde{B})$ are computed by the cluster characters
  ...
  \begin{proof}
    ...
    \begin{equation}
      ...
      \xymatrix{ & & 0\ar[d] & 0\ar[d] &\\ 0\ar[r]& V'\ar[r]\ar@{=}[d]& \widetilde{E}\ar[d]\ar[r]\ar@{}[dr]|<{\lrcorner}& V\ar[d]^{\iota_0}\ar[r]& 0\\ 0\ar[r]& V'\ar[r]& E\ar[r]\ar[d]& J_0\ar[r]\ar[d]& 0\\ & & J_1\ar[d]\ar@{=}[r] & J_1\ar[d] &\\ & & 0 & 0 &.}
    \end{equation}
  \end{proof}
}
  ...
  \begin{theorem}\cite{rupel1}
    ...
    Let $k$ be a sink or a source in $Q$ and write $(\bfX',\widetilde{B}')=\mu_k(\bfX,\widetilde{B})$.  
