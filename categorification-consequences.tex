...
%%%%%%%%%%%%%%%%%%%%%%%%
  ...
  We are grateful to Giovanni Cerulli-Irelly for some insight on references.
  ...
  Let $B$ be a $n\times $ skew-symmetrizable integer matrix and assume that it is \neword{acyclic} (i.e. up to a simultaneous permutation of rows and columns $B$ has only non-negative entries above the diagonal).
  Let $\widetilde{B}$ be the matrix obtained by stacking a $n\times n$ identity matrix below $B$ and write $\cA(\bfx,\widetilde{B})$ for the associated cluster algebra; it a cluster algebra with \newword{principal coefficients}.

  Since $\widetilde{B}$ has full rank, the algebra $\cA(\bfx,\widetilde{B})$ admits a log-canonical Poisson structure compatible with mutations \cite{gekhtman-shapiro-vainshtein} and one may obtain a quantum cluster algebra via a remarkably simple deformation quantization.  
